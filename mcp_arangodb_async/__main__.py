...
from .cli import db_arango as cli_db_arango
from .cli import health as cli_health
from .cli import user as cli_user
from .cli import arango as cli_arango
...
def main() -> int:
    ...
    # Arango subcommand - Unified V4 tool CLI interface
    arango_parser = subparsers.add_parser(
        "arango",
        help="Execute V4 tool operations directly (database/collection/view/graph/aql/mcp)",
    )
    arango_parser.add_argument(
        "arango_tool",
        choices=["database", "collection", "view", "graph", "aql", "mcp", "template"],
        help="V4 tool to use",
    )
    arango_parser.add_argument(
        "arango_action",
        help="Operation to perform (e.g., list, insert, query)",
    )
    # Common arguments for arango operations
    arango_parser.add_argument("--collection", "-c", help="Collection name")
    arango_parser.add_argument("--data", "-d", help="Document data (JSON string)")
    arango_parser.add_argument("--filter", "-f", help="Filter conditions (JSON string)")
    arango_parser.add_argument("--key", "-k", help="Document key")
    arango_parser.add_argument("--query", "-q", help="AQL query string")
    arango_parser.add_argument("--limit", "-l", type=int, help="Result limit")
    # Template arguments
    arango_parser.add_argument("--name", "-n", help="Template name (e.g., memory.recent)")
    arango_parser.add_argument("--params", "-p", help="Template parameters (JSON string)")
    # Backup/restore arguments
    arango_parser.add_argument("--output-dir", "--output", "-o", dest="output_dir", help="Backup output directory")
    arango_parser.add_argument("--input-dir", "--input", "-i", dest="input_dir", help="Restore input directory")
    arango_parser.add_argument("--collections", help="Collections to backup/restore (comma-separated)")
    arango_parser.add_argument("--type", "-t", choices=["collection", "graph", "view", "all"], help="Backup/restore type")
    arango_parser.add_argument("--conflict", choices=["skip", "replace", "update"], help="Restore conflict mode")
    ...
    # Handle arango subcommand
    if args.command == "arango":
        return cli_arango.handle_arango_command(args)
