...
> Fork note: this repository is maintained as a production-focused fork. It keeps a general-purpose MCP toolkit baseline and adds a forward-looking, optional tags graph intelligence workflow.
...
🔧 **ArangoDB Setup:** [docs/getting-started/install-arangodb.md](docs/getting-started/install-arangodb.md)

🗄️ **Multi-Tenancy Guide:** [docs/user-guide/multi-tenancy-guide.md](docs/user-guide/multi-tenancy-guide.md)

⚙️ **CLI Reference:** [docs/user-guide/cli-reference.md](docs/user-guide/cli-reference.md)

📖 **Tools Reference:** [docs/user-guide/tools-reference.md](docs/user-guide/tools-reference.md)

🧠 **Tags Auto Graph Guide:** [docs/user-guide/tags-graph-auto.md](docs/user-guide/tags-graph-auto.md)

📝 **Changelog:** [docs/developer-guide/changelog.md](docs/developer-guide/changelog.md)
...
✅ **Unified MCP Toolkit** - Database, collection, view, graph, admin, and MCP metadata tooling
...
✅ **Tags Auto Graph (Optional)** - Daily `sync_run` and log-driven `optimize_run` for dynamic tag-edge intelligence
...
## Tags Auto Graph (Optional, Future Direction)

The fork introduces a practical tags graph loop that can run without changing your core CRUD/AQL usage.

- `sync_run`: build and refresh `tags` + `tag_edges` from `notes.tags` using `AND/OR/NOT/XOR`
- `optimize_run`: adjust `tag_edges.weight` and `enabled` based on `access_logs` behavior signals
- both actions are exposed via one admin entry: `arango_admin`

Minimal calls:

```json
{ "action": "sync_run", "dry_run": true }
```

```json
{ "action": "optimize_run", "days": 30, "dry_run": true }
```

Runtime defaults are controlled in `config/admin.json`.
For full workflow and parameter reference, see [docs/user-guide/tags-graph-auto.md](docs/user-guide/tags-graph-auto.md).
